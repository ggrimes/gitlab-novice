...
When using Git, you often have files that you **don’t want to track**.
These could be:
...
Git provides a file named `.gitignore` where you can list patterns that
match files or directories Git should ignore.
...
No. `.gitignore` only affects **new** files. If you've already added a
file to Git, `.gitignore` won’t stop it from being tracked. To remove a
...
If you want to ignore files only on your machine (not share with
others), you can use:
...
This works like `.gitignore`, but it’s **not committed** to the
repository.

::: keypoints
-   Use `.gitignore` to prevent tracking of unnecessary or sensitive
    files
